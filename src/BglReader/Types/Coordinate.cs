    ...
    Latitude latitude) : IEquatable<Coordinate>
{
    ...
    internal static Coordinate FromBgl(int longitude, int latitude, int elevation) => new(Longitude.FromDword(longitude), Latitude.FromDword(latitude), Types.Elevation.FromBgl(elevation));
